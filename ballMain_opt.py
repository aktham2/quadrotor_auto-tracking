...
# orientation sensor for y,z rotation (roll,pitch)
import bno
...
def rotateYZ(pos):
	# using BNO055, rotate the x,y vector to the ball into the absolute frame (Ryz)^-1
	rotY = bno.getRotY()
	rotZ = bno.getRotZ()
	s1 = np.sin(rotY)
	c1 = np.cos(rotY)
	s2 = np.sin(rotZ)
	c2 = np.cos(rotZ)
	return np.array([[c1*c2, -c1*s2, s1],[s2, c2, 0], [-s1*c2, s1*s2, c1]]).T @ np.array(pos)
	
	
def ball(oldLoc):
	# optimal processing frame size
	procSize = 200/2
	...
	# crop image for faster processing to estimated new position
	frame = frame[oldLoc[0]-procSize:oldLoc[0]+procSize, oldLoc[1]-procSize:oldLoc[1]+procSize]
    ...
    if len(cnts) > 0:
        ...
        if radius > 10:
			# transform back to original big frame coordinates
			x = x - procSize + oldLoc[0]
			y = y - procSize + oldLoc[1]
            return rotateYZ([x,y]), radius
    return (0,0),0
...
##        trackBall = False
    ...
    while trackBall:
        try:
            ballLocation,ballRadius = ball(ballLocation)
            print("")
            if ballRadius != 0:
